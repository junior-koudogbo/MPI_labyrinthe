...
void initgraph(int larg, int haut) {
  ...
  /* on crée la fenêtre */
  window = XCreateSimpleWindow(display, RootWindow(display, screen), 0, 0, larg,
                               haut, BORDER_WIDTH, black, white);
  ...
  XpmCreatePixmapFromData(display, window, icon_xpm, &icon_pixmap,
                          NULL, NULL);
  */
  icon_pixmap = XCreateBitmapFromData(display, window, icon_bits, icon_width,
                                      icon_height);
  ...
  XSetWMProperties(display, window, &windowName, &iconName, NULL, 0, size_hints,
                   wm_hints, NULL);
  XFree(wm_hints);
  ...
  */
  ...
  XChangeWindowAttributes(display, window, CWBackingStore | CWSaveUnder,
                          &wattr);

  shadow =
      XCreatePixmap(display, window, larg, haut, DefaultDepth(display, screen));
  ...
  do {
    ...
    switch (report.type) {
      case Expose:
        ...
        XCopyArea(display, shadow, window, gc, 0, 0, wattr.width, wattr.height,
                  0, 0);
...
Bool *check_keys(void) {
  ...
  if (XCheckMaskEvent(display, ExposureMask, &report)) {
    XGetWindowAttributes(display, window, &wattr);
    XCopyArea(display, shadow, window, gc, 0, 0, wattr.width, wattr.height, 0,
              0);
  }
